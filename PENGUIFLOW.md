...
> Generated for **echo-agent** with PenguiFlow v2.7
...
```bash
# 1. Edit your spec
# Open echo-agent.yaml and customize tools, prompts, etc.
...
uv run python -m echo_agent "Your query here"
```
...
agent:
  ...
  flags:
    ...
    a2a: false                # A2A HTTP+JSON binding
```
...
**A2A binding:**
If `agent.flags.a2a` is enabled, the scaffold includes an `a2a.py` module that wires
`A2AService` + `create_a2a_http_app` so you can serve the agent over A2A HTTP+JSON.
...
```python
# src/echo_agent/tools/search_documents.py
...
LLM_MODEL=gpt-4o uv run python -m echo_agent "Query"
```
